...
public class Mapper4ORM {
    ...
    public BaseExample getExample(Class mapperInterface) throws Exception {
        ParameterizedType t = (ParameterizedType) mapperInterface.getGenericInterfaces()[0];//得到目标类实现的接口对应的Type对象
    ...
    /**
     * 获取一个类里的，有指定annotation的，所有 Field
     *
    ...
    List<Field> getFieldsEquals(Class objectClass, Class annotationClass) {
        ...
        for (Class temp = objectClass; temp != Object.class; temp = temp.getSuperclass()) {//temp != Object.class 结果为true时继续执行，为false不执行
            fields.addAll(Arrays.asList(temp.getDeclaredFields()));//数组转集合
        }
        /*
    ...
    public void fillEnumOnWriting(Object object) throws Exception {
        ...
        Class clazz = object.getClass();
        // 获取所有Enumerated 注解的Field
        List<Field> result = getFieldsEquals(clazz, Enumerated.class);
        for (Field field : result) {
            //获取Enum对应的，String类型的变量名     group_
            String varName = field.getAnnotation(Enumerated.class).var();
            //获取 Enum
            Enum enumObj = (Enum) clazz.   //field.getName()= group ; getMethod=getGroup()  传入值：User.Group.user
                    getMethod("get" + StringUtils.capitalize(field.getName())).invoke(object);

            // 转成 String，插回 varName
            String enumString = enumObj.name();  //枚举值 user
            clazz.getMethod("set" + StringUtils.capitalize(varName), String.class)//将setGroup_()方法中的group_赋值为user
