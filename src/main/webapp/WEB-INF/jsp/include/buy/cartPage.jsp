...
<main class="cart">
    <form method="post" id="cartForm" action = "${pageContext.request.contextPath}/buy">
        <div class="sum-top">
        ...
        <table class="cart-list">
            ...
            <c:forEach items="${cartItems}" var="item">
                <tr select="false" stock="${item.product.stock}" buy="${item.number}"
                    price-per="${item.product.nowPrice}" ciid="${item.id}">
                    <input type="hidden" name="ciid" value="${item.id}" id="ciid">
                    <td>
                        <img src="img/noselect.png" class="select-img">
                        <a href="${pageContext.request.contextPath}/product?id=${item.product.id}"><img
                                src="${productImgDir}${item.product.image.path}" class="cart-item-jpg"></a>
                    </td>
                    <td class="item-name">
                    ...
                    <td>
                        <div class="cart-change">
                            <a style="cursor: pointer" class="cart-decrease cart-change-button">-</a><input
                                value="${item.number}" class="item-num-input"><a style="cursor: pointer"
                                                                                 class="cart-increase cart-change-button">+</a>
                        </div>
                    </td>
                    <td>
                    ...
                    <td>
                        <a style="cursor: pointer" class="delete-button" ciid="${item.id}">删除</a>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
        ...
        <div class="sum-bottom  clear">
            ...
            <span>
                ...
                <button class="crate-order-big crate-order" id="countMoney" type="submit">结 算</button>
            </div>
        </div>
    </form>
