...
<section class="order-type">
    <div class="selected"><a style="cursor: pointer" order-status="all">所有订单</a></div>
    <div><a style="cursor: pointer" order-status="waitPay">待付款</a></div>
    <div><a style="cursor: pointer" order-status="waitDelivery">待发货</a></div>
    <div><a style="cursor: pointer" order-status="waitConfirm">待确认</a></div>
    <div><a style="cursor: pointer" order-status="waitComment">待评价</a></div>
</section>
    ...
    <c:forEach items="${orders}" var="o" varStatus="vs">
        <table class="line-table" order-status="${o.status}" oid="${o.id}">
            <tr class="item-head">
                ...
                <td class="delete">
                    <c:if test="${o.status=='finish'|| o.status == 'waitPay'}">
                        <a style="cursor: pointer"  class="delete-button" oid="${o.id}">
            ...
            <c:forEach items="${o.orderItems}" var="oi" varStatus="vs2">
                <tr class="item-body">
                    <td width="100px">
                        <a href="${pageContext.request.contextPath}/product?id=${oi.product.id}"><img
                                src="${productImgDir}${oi.product.image.path}" class="cart-item-jpg"></a>
                    </td>
                    <td class="item-name">
                        <a class="cart-item-title"
                           href="${pageContext.request.contextPath}/product?id=${oi.product.id}">${oi.product.name}</a>
                    ...
                    <c:if test="${vs2.count==1 && o.status!='waitComment' }">
                        <td width="120px" rowspan="${o.totalNumber}">
                            ...
                            <c:if test="${o.status=='waitConfirm' }">
                                <a href="${pageContext.request.contextPath}/confirmPay?oid=${o.id}" class="order-button blue">确认收货</a>
                            </c:if>
                            <c:if test="${o.status=='waitPay' }">
                                <a href="${pageContext.request.contextPath}/pay?oid=${o.id}" class="order-button blue">付款</a>
                            </c:if>

                            <c:if test="${o.status=='waitDeliver' }">
                                <a style="cursor: pointer" class="order-button white">待发货</a>
                                <a href="${pageContext.request.contextPath}/deliver?oid=${o.id}" class="order-button blue">自己发货</a>
                            </c:if>
                            <c:if test="${o.status=='finish' }">
                                <a style="cursor: pointer"  class="order-button white">完成订单</a>
                            </c:if>
                        </td>
                    </c:if>
                    ...
                    <c:if test="${o.status=='waitComment' && (empty oi.comment)}">
                        <td width="120px">
                            <a href="${pageContext.request.contextPath}/comment?oiid=${oi.id}&oid=${o.id}" class="order-button white">评价</a>
                        </td>
                    </c:if>
                </tr>
            </c:forEach>
